...
def plot_markers(lons, lats, marker="k*", transform=None):
    ...
    if (type(lons) == int) | (type(lons) == float):
        lons = np.array([lons])
    if (type(lats) == int) | (type(lats) == float):
        lats = np.array([lats])
...
def maps(
    ...
    fix_ncolumns=False,
    fix_nrows=False,
    order_subplots_across_columns=False,
    shared_levels=False,
    hide_colbars=False,
    shared_colbar=False,
    ...
    * labels: Panel labelling (default is None)
    ...
    ncubes = len(cubes)
    if ncubes > 60:
        raise ValueError("Soft limit reached: too many cubes to plot at once")
    ncolumns, nrows = auto_define_subplot_layout(
        ncubes, fix_ncolumns=fix_ncolumns, fix_nrows=fix_nrows
    )
    ...
    for i, c in enumerate(cubes):
        ...
        if region_mask != None:
            ...
            c = _region_mask(
                c, region_mask
            )  ### is it better to run this once for all cubes???
            if mask_ocean == None:
                mask_ocean = True
        ...
        if plot_type == "pcolormesh":
            im = iplt.pcolormesh(
                c, cmap=cmap, vmin=min_val, vmax=max_val
            )  # , edgecolors='w', linewidths=0.3)

        ### Plot p-values
        if cube_p_values != None:
            iplt.contour(
                cube_p_values[i], [0, p_value_level], colors=p_value_contour_color
            )
        ...
        if (show_coastlines == True) & (show_iceshelves == False):
            add_feature(
                cartopy.feature.COASTLINE, linewidth=0.7
            )  # plt.gca().coastlines()
        if mask_ocean == True:
        ...
        units_label = c.units
        if units_label == "degreesC":
            units_label = r"$\degree$C"
        ...
        if labels != False:
            ...
            plt.gca().add_artist(
                AnchoredText(labels[i], loc=2, borderpad=0.0, prop=dict(size=8.5))
            )
    ...
    if (
        shared_colbar == True
    ):  ### TO DO: only add when all plots have matching units + levels !!!!
        width = np.max(left_positions) - np.min(left_positions) + width
        colorbar_axes = fig.add_axes(
            [np.min(left_positions), bottom / 1.5, width, 0.025]
        )
    ...
    if add_author == True:
        _add_author()
    if type(add_author) == str:
        _add_author(add_author)
    if type(add_source) == str:
