...
def setup_catalogue_file(
    dataset,
):  # this has been stripped down compared to bp.catalogue (remove slow requests to http)
    """
    ...
    if os.path.isfile(cat_file) == False:
        ...
        if platform.system() in ["Darwin", "Linux"]:
            raise ValueError(
                "File does not exist. Run the following line in your Mac/Linux terminal: \n"
                + " wget "
                + __catalogues_url
                + cat_fname
                + " -O "
...
def read_csv_with_comments(fname):
    ...
    if "dtypes" in dataset_dict.keys():
        ...
        df = read_csv(
            fname, comment="#", dtype=dtypes
        )  # should we be using chunksize to speed things up? !!
    else:
    ...
    df = df.astype(
        {"dataset": "category"}
    )  # can we do this in one step, with line above? !!
...
def __create_unique_run_identifer(catlg, col_name):
    ...
    my_list.remove("")
    if (
        "Version!latest" in my_list
    ):  # plan to remove !latest from datasets.py (should read all data then take newest version where all Vars exist)
...
def __complete_var_set(catlg, filt_dict):
    ...
    if len(filt_dict["Var"]) < 2:
        raise ValueError(
            "Two or more Vars need to be specified for complete_var_set to work"
        )
    ...
    catlg = catlg[
        catlg.isin({"Unique_Model_Run": catlg_gp.index})["Unique_Model_Run"] == True
    ]

    if len(catlg) == 0:
        raise ValueError(
            "There are no rows where all specified Vars exist for Model-RunID-Version",
            filt_dict["Var"],
        )
...
def catalogue(
    ...
    if (complete_var_set == True) & ("Var" not in user_values.keys()):
        raise ValueError(
            "complete_var_set only works when you specify two or more variables (Vars)"
        )
                ...
                for v in pd.unique(cat["Var"]):
                    ...
                    if len(pd.unique(cat_tmp["SubModel"])) > 1:
                        raise ValueError(
                            ...
                            + str(pd.unique(cat_tmp["SubModel"]))
                        )
