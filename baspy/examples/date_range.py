...
netcdfs = [
    "tasmax_Aday-ISIMIP2b_CAM4-2degree_Plus20-Future_CMIP5-MMM-est1_v1-0_run001_19990101-20091231.nc4",
...
if (start_date != None) | (end_date != None):
    ...
    if start_date == None:
        start_date = np.min(file_date_range[0])
    if end_date == None:
        end_date = np.max(file_date_range[1])

    keep_ind1 = np.where(
        (end_date >= file_date_range[0]) & (end_date <= file_date_range[1])
    )[0]
    keep_ind2 = np.where(
        (start_date >= file_date_range[0]) & (start_date <= file_date_range[1])
    )[0]
