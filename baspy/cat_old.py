...
def setup_catalogue_file(dataset):
    ...
    if os.path.isfile(cat_file) == False:
        print(
            cat_file
            + " does not exist, this may be the first time you've requested this catalogue"
        )
        get_file = True

    ### 4. no trace of catalogue file, lets build one
    requests.get(
        __shared_url_cat_file, timeout=20
    )  # check we have connection - stops the users terminal hanging...
    ...
    if os.path.exists(__shared_local_cat_file):
        ...
        url_file_timestamp = get_last_modified_time_from_http_file(
            __shared_url_cat_file
        )
    ...
    if newer_available_location != None:
        warnings.warn(
            "Using catalogue "
            + cat_file
            + ". Note that a newer version is available at "
            + newer_available_location
        )

    return (
        force_catalogue_refresh,
        copied_new_cat_file,
        cat_file,
        __shared_local_cat_file,
    )
...
def read_csv_with_comments(fname):
    ...
    df = read_csv(
        fname, comment="#"
    )  # define dtypes here to reduce memory usage (see df.memory_usage() and df.dtypes())!
    df["dataset"] = metadata["dataset"]
    df = df.astype(
        {"dataset": "category"}
    )  # can we do this in one step to limit initial memory usage from line above? (see df.memory_usage())

    if __catalogue_version > int(metadata["catalogue_version"]):
        raise ValueError(
            "Your catalogue needs to be updated to work with this version of the code"
        )
...
def __refresh_shared_catalogue(dataset):
    ...
    (
        ...
        cat_file,
        __shared_cat_file,
    ) = setup_catalogue_file(dataset)

    if dataset not in dataset_dictionaries.keys():
        raise ValueError(
            "The keyword 'dataset' needs to be set and recognisable in order to refresh catalogue"
        )
    ...
    for path in paths:
        ...
        if len(fnames) > 0:
            start_date, end_date = get_file_date_ranges(
                fnames, dataset_dict["FilenameStructure"]
            )
    ...
    df = pd.DataFrame(
        rows, columns=DirStructure + ["StartDate", "EndDate", "Path", "DataFiles"]
    )
        ...
        if ("_fx_" in fname) | ("_Efx_" in fname) | ("_Ofx_" in fname):
            ...
            if len(date_str) == 2:
                ...
                if date_str[2] == "clim":
                    ...
                    start_date, end_date = int(date_str[0] + "01"), int(
                        date_str[1] + "31"
                    )
                else:
...
def __combine_dictionaries(keys, dict1_in, dict2_in):
    ...
    for key in keys:
        ...
        if dict1[key].__class__ == str:
            dict1[key] = [dict1[key]]
        if dict1[key].__class__ == np.string_:
            dict1[key] = [dict1[key]]
        if dict2[key].__class__ == str:
            dict2[key] = [dict2[key]]
        if dict2[key].__class__ == np.string_:
            dict2[key] = [dict2[key]]
...
def __filter_cat_by_dictionary(cat, cat_dict, complete_var_set=False):
    ...
    for key in keys:
        ...
        if cat_dict[key].__class__ == str:
            cat_dict[key] = [cat_dict[key]]
        if cat_dict[key].__class__ == np.string_:
            cat_dict[key] = [cat_dict[key]]
        ...
        for val in vals:
            if val not in uniq_vals:
                print(
                    "Are you sure that data exists that satisfy all your constraints?"
                )
                raise ValueError(
                    val
                    + " not found. See available in current catalouge: "
    ...
    if complete_var_set == True:
        ...
        if "Var" not in keys:
            raise ValueError(
                "Two or more Varaibles (Var=) need to be specified in order to use complete_var_set"
            )
        ...
        print(
            "Filtering catalogue to provide a complete set for variables: ",
            cat_dict["Var"],
        )
        ...
        for i in other_keys:
            if len(cat_dict[i]) > 1:
                raise ValueError(
                    "complete_var_set: only one item allowed for keys other than Var. You have: "
                    + i
                    + "="
                    + str(cat_dict[i])
                )
        ...
        print(
            "complete_var_set=True: Adding 'Unique_Model_Run' as a new column to the catalogue"
        )
        cat["Unique_Model_Run"] = (
            cat["Centre"]
            + "_"
            + cat["Model"]
            + "_"
            + cat["RunID"].astype(str)
            + "_"
            + cat["Experiment"]
        )
...
def __compare_dict(dict1_in, dict2_in):
    ...
    for key in uniq_keys:
        ...
        if dict1[key] != dict2[key]:
            compare_dicts = "different"

    return compare_dicts


def catalogue(
    dataset=None, refresh=None, complete_var_set=False, read_everything=False, **kwargs
):
    """
    ...
    if dataset == None:
        print(
            "Warning: dataset not specified, defaulting to: dataset='"
            + __default_dataset
            + "'"
        )
    ...
    if dataset not in dataset_dictionaries.keys():
        raise ValueError(
            dataset
            + " dataset not currently available: "
            + str(dataset_dictionaries.keys())
    ...
    expanded_cached_values = __combine_dictionaries(
        __cached_values.keys(), __cached_values, user_values
    )
    compare_dicts = __compare_dict(expanded_cached_values, __cached_values)
    if compare_dicts == "different":
        update_cached_cat = True
    ...
    if user_values != {}:
        ...
        cat = __filter_cat_by_dictionary(
            __cached_cat, user_values, complete_var_set=complete_var_set
        )
                ...
                for v in np.unique(cat["Var"]):
                    ...
                    if len(np.unique(cat_tmp["SubModel"])) > 1:
                        raise ValueError(
                            "Var='"
                            + v
                            + "' maybe ambiguous, try defining Submodel"
                            + "\n SubModel values available: "
                            + str(np.unique(cat_tmp["SubModel"]))
                        )
        ...
        if "Frequency" in cat.columns:
            if len(np.unique(cat["Frequency"])) > 1:
                raise ValueError(
                    "Multiple time Frequencies present in catalogue, try defining Frequency"
                    + "\n Frequency values available: "
                    + str(np.unique(cat["Frequency"]))
                )
        if "CMOR" in cat.columns:
            if len(np.unique(cat["CMOR"])) > 1:
                raise ValueError(
                    "Multiple CMOR values present in catalogue, try defining CMOR"
                    + "\n CMOR values available: "
                    + str(np.unique(cat["CMOR"]))
                )
        ...
        if complete_var_set == True:
            raise ValueError(
                "Can not specify complete_var_set when less than two variables (Var) are defined"
            )
...
def get_files(df):
    ...
    if len(np.unique(list_file_extensions)) > 1:
        ...
        raise ValueError(
            ">> WARNING: Multiple file extensions present in " + directory + " <<"
        )
