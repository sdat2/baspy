            ...
            for cube in cubelist:
                ...
                for var_name in var_names:
                    if var_name == "latitude":
                        cube.coord(var_name).var_name = "lat"
                    if var_name == "longitude":
                        cube.coord(var_name).var_name = "lon"
        ...
        if model == "EC-EARTH":
                        ...
                        if time_coord.units.is_time_reference():
                            if time_coord.units.calendar == u"gregorian":
                                time_coord.units = cf_units.Unit(
                                    time_coord.units.origin, u"standard"
                                )
            ...
            for cube in cubelist:
                for time_coord in cube.coords():
                    if time_coord.units.is_time_reference():
                        if (
                            time_coord in cube.aux_coords
                            and time_coord not in cube.dim_coords
                        ):
            ...
            for cube in cubelist:
                ...
                for axis in coords:
                    if cube.coord(axis).points.dtype == "float64":
                        cube.coord(axis).points = cube.coord(axis).points.astype(
                            "float32"
                        )
                for axis in ["time", "latitude", "longitude"]:
                    if cube.coord(axis).bounds.dtype == "float64":
                        cube.coord(axis).bounds = cube.coord(axis).bounds.astype(
                            "float32"
                        )
