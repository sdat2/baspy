...
def catalogue(refresh=None, **kwargs):
    ...
    for name in names:
        ...
        if vals.__class__ == str:
            vals = [vals]
        for val in vals:
            if val not in uniq_label:
                raise ValueError(
                    val + " not found. See available: " + np.array_str(uniq_label)
                )
    ...
    for i in range(0, len(filt_cat)):
        ...
        for nc in netcdfs:
            ...
            if str(filt["Stash"]) not in nc:
                print(
                    ">> Information: Detected misplaced file"
        ...
        for j in netcdfs:
            ...
            cube = iris.load(
                dirfilename, constraints=constraints, callback=upscale_callback
            )
