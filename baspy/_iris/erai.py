...
def get_erai_6h_cat(start_date, end_date, level=None, verbose=True):
    ...
    if level == None:
        print(
            "'level' undefined, setting to 'as' (surface level data). \n"
...
def level_from_var_name(var_name):
    ...
    if var_name in df_as["surface_vars"].values:
        level_str = "as"
    if var_name in df_ap["pressure_lev_vars"].values:
        level_str = "ap"
    if level_str == None:
...
def get_cube(
    start_date, end_date, var_name, frequency="6hr", constraints=None, verbose=True
):
    ...
    with units.suppress_errors():
        cubelist = iris.load(fnames, constraints=con, callback=edit_erai_attrs)

    ### Setup new coord to add to cube (see below)
    model_coord = iris.coords.AuxCoord(
        "ERA-Interim", long_name="Model", units="no_unit"
    )
...
def get_land_mask():
    with units.suppress_errors():
        cube = iris.load_cube(
            root + "/era-interim/erai_invariant.nc", "land_binary_mask"
        )
    return cube
