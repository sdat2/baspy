    ...
    if ("lon_bnds" in keys) | ("lat_bnds" in keys):
        ...
        if ("lon_bnds" in keys) & ("lat_bnds" in keys):
            cube = cube.intersection(
                longitude=bounds["lon_bnds"], latitude=bounds["lat_bnds"]
            )
...
def cube_trend(cube, var_name=None, time_coord=None):
    ...
    if len(years) == 0:
        raise ValueError(
            "Need to assign either 'year' or 'season_year' as Auxiliary coordinates to cube"
        )
    ...
    for x in range(0, len(lons)):
        for y in range(0, len(lats)):
            (
                ...
                p_value.data[y, x],
                std_err,
            ) = stats.linregress(years, cube.data[:, y, x])
...
def months2seasons(cube, seasons=None):
    ...
    for i in range(0, ntimes):
        ind = np.where(
            (clim_seasons == clim_seasons[i]) & (season_years == season_years[i])
        )[0]
        n_months_in_season = len(
            clim_seasons[i]
        )  # length of string, usually 3 (e.g. 'djfm'=4)
        if len(ind) == n_months_in_season:
            keep_ind = np.append(keep_ind, i)
    time_axis = cube.coord_dims(cube.coord(axis="t"))[0]
    cube_slice = (
        (slice(None),) * time_axis
        + (keep_ind,)
        + (slice(None),) * (len(cube.shape) - time_axis - 1)
    )
...
def months2annual(cube):
    ...
    if len(cube.coords("month")) == 1:
        cube.remove_coord("month")
    if len(cube.coords("clim_season")) == 1:
        cube.remove_coord("clim_season")
    if len(cube.coords("season_year")) == 1:
...
def unify_similar_grid_coords(cubes, cube_template=None):
    ...
    if (len(cube_template.coords(axis="X")) == 1) & (
        len(cube_template.coords(axis="Y")) == 1
    ):
        ...
        if len(cubes) > 0:
            for i, cube in enumerate(cubes):
                for axis in ["X", "Y"]:
                    ...
                    if np.any(
                        cube.coord(axis=axis).points
                        != cube_template.coord(axis=axis).points
                    ) and np.all(
                        np.isclose(
                            cube.coord(axis=axis).points,
                            cube_template.coord(axis=axis).points,
                        ...
                        cube.coord(axis=axis).points = cube_template.coord(
                            axis=axis
                        ).points
                        cubes[i] = cube

                    if "NoneType" in str(type(cube.coord(axis=axis).bounds)):
                        cube.coord(axis=axis).bounds = cube_template.coord(
                            axis=axis
                        ).bounds
                        cubes[i] = cube

                    if np.any(
                        cube.coord(axis=axis).bounds
                        != cube_template.coord(axis=axis).bounds
                    ) and np.all(
                        np.isclose(
                            cube.coord(axis=axis).bounds,
                            cube_template.coord(axis=axis).bounds,
                    ...
                    if axis == "X":
                        cube.coord(axis=axis).circular = cube_template.coord(
                            axis=axis
                        ).circular
                        cubes[i] = cube
        ...
        print(
            "WARNING: Can not attempt to unify similar cube coordinates as more than one coord associated with axis='X' and/or axis='Y' "
        )
...
def rm_time_overlaps(cubelist):
    ...
    while i < len(cubelist):
        ...
        if min2 <= max1:
            print(">>> WARNING: Removing temporal overlaps" " from cubelist <<<")
            print(min2, max1)
...
def eg_cubelist():
    ...
    with warnings.catch_warnings():
        ...
        cubelist = iris.load(
            file, ["air_temperature", "eastward_wind", "precipitation_flux"]
        )
...
def create_ensemble_cube(cubelist, coord_labels, long_name=None, units=None):
    ...
    for i, cube in enumerate(cubelist):
        ...
        new_coord = iris.coords.AuxCoord(
            coord_labels[i], long_name=long_name, units=units
        )
...
def make_ts_cube(ts, cube, rename=None, units=None):
    ...
    for coord in cube.aux_coords:
        if len(coord.points) == 1:
            data_dims = None
        if len(coord.points) == len(t_coord.points):
            data_dims = 0  ### Generalise this!!!!!!!!!!!!!!!!
...
def region_mask(cube, region_name):
    ...
    country_crit = (
        lambda country: country.attributes["name_long"] == region_name
    )  ## e.g., 'China'
