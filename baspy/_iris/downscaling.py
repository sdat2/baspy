...
def _create_cube(arr, template_cube, deepcopy=True, add_attr=None):
    ...
    if (type(template_cube.data) == np.ma.core.MaskedArray) & (type(arr) == np.ndarray):
...
def bias_correction(Obs_ref, Sim_ref, Sim_fut):
    ...
    bc = Obs_ref_mean.data + (Obs_ref_std.data / Sim_ref_std.data) * (
        Sim_fut.data - Sim_ref_mean.data
    )
...
def change_factor(Obs_ref, Sim_ref, Sim_fut):
    ...
    cf = Sim_fut_mean.data + (Sim_fut_std.data / Sim_ref_std.data) * (
        Obs_ref.data - Sim_ref_mean.data
    )
...
def qm_bias_correction(obs_ref, sim_ref, sim_fut):
    ...
    for xy_ind in np.ndindex(collapsed_cube.shape):
        t_slice = (
            xy_ind[:time_axis] + (slice(None),) + xy_ind[time_axis:]
        )  # e.g., (slice(None, None, None), 1, 1)
        ...
        for i, val in enumerate(sim_fut.data[t_slice]):
            ...
            bias = np.percentile(obs_ref.data[t_slice], percentile) - np.percentile(
                sim_ref.data[t_slice], percentile
            )
            tyx_ind = (
                xy_ind[:time_axis] + (i,) + xy_ind[time_axis:]
            )  # e.g., (2699, 1, 1)
            bias_correction[tyx_ind] = val + bias

    ### Add attribute for calibration method
    bias_correction = _create_cube(
        bias_correction,
        sim_fut,
...
def qm_change_factor(obs_ref, sim_ref, sim_fut):
    ...
    for xy_ind in np.ndindex(collapsed_cube.shape):
        ...
        for i, val in enumerate(obs_ref.data[t_slice]):
            ...
            cf = np.percentile(sim_fut.data[t_slice], percentile) - np.percentile(
                sim_ref.data[t_slice], percentile
            )
            tyx_ind = (
                xy_ind[:time_axis] + (i,) + xy_ind[time_axis:]
