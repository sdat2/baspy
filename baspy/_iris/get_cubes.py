...
def callback(cube, field, filename):
    ...
    for i in range(0, len(split_str)):
        if long_names[i] in whitelist_auxcoords:
            new_coord = coords.AuxCoord(
                split_str[i], long_name=long_names[i], units="no_unit"
            )
    ...
    if len(cube.coords(axis="t")) > 0:
        ...
        iris.coord_categorisation.add_season(
            cube, time_name, name="clim_season", seasons=seasons
        )
        iris.coord_categorisation.add_season_year(
            cube, time_name, name="season_year", seasons=seasons
        )
...
def get_cubes(filt_cat, constraints=None, verbose=True, nearest_lat_lon=False):
    ...
    for index, row in filt_cat.iterrows():
        ...
        for d in datafiles:
            if (
                (my_dict["Model"] not in d)
                | (my_dict["Var"] not in d)
                | (my_dict["RunID"] not in d)
                | (my_dict["Experiment"] not in d)
            ):
                raise ValueError(
                    ">> WARNING: Detected misplaced files in " + path + " <<"
                )
        ...
        if len(np.unique(list_file_extensions)) > 1:
            raise ValueError(
                ">> WARNING: Multiple file extensions present in " + path + " <<"
            )
        ...
        for datafile in datafiles:
            ...
            con = iris.Constraint(
                cube_func=lambda cube: cube.var_name == my_dict["Var"]
            )
            ...
            if len(cube) > 1:
                raise ValueError("more than one cube loaded, expected only one!")
        ...
        ### Unify lat-lon grid
        tmp_cubelist = unify_similar_grid_coords(
            tmp_cubelist, tmp_cubelist[0]
        )  # !! issue with ocean variables

        ### Apply Fixes to enable cubes in tmp_cubelist to concatenate ###
        tmp_cubelist = fix_cubelist_before_concat(
            tmp_cubelist, __current_dataset, my_dict
        )
...
def get_cmip5_template_cube():
    ...
    con = (
        iris.Constraint(cube_func=lambda cube: cube.var_name == "tas")
        & iris.Constraint(year=2000)
        & iris.Constraint(month=1)
    )
...
def get_fx(model, Var):
    ...
    if len(exps) == 0:
        raise ValueError("No " + Var + " files exists for " + model)
    ...
    for wl in whitelist:
        if wl in exps:
            fx = get_cubes(
                catalogue(Model=model, Frequency="fx", Var=Var, Experiment=wl)
            )
            if len(fx) > 1:
                print(
                    "Warning: more than one " + Var + " file found.  Using first one."
                )
            return fx[0]
